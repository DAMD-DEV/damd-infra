...
@section('main')
    <h1>工場</h1>
    <div class="clearfix">
        <a href="{{ route('admin.factories.create') }}" class="btn btn-primary float-end">追加</a>
    </div>
    <div>
        <h2>工場一覧</h2>
        {{ $factories->links() }}
        <div class="main-list list-group">
            @foreach($factories as $factory)
                <div href="" class="list-group-item list-group-item-action" data-bs-toggle="modal" data-bs-target="#testModal" data-factory-id={{ $factory->id }} data-name="{{ $factory->name }}" data-day="{{ $factory->day }}" data-display="{{ $factory->display }}">
	...
	<script>
		$(function(){
			$('.list-group-item').click(function(){
				...
				$edit_url = @json(route('admin.factories.edit', 'number'));
				$edit_url = $edit_url.replace('number', $factory_id);
				$('#testModal').find('.edit-btn').attr('href', $edit_url);

				$delete_url = @json(route('admin.factories.destroy', 'number'));
