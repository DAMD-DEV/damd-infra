...
@section('main')
    <h1>送料区分</h1>
    <div class="clearfix">
        <a href="{{ route('admin.shippings.create') }}" class="btn btn-primary float-end">追加</a>
    </div>
    <div>
        <h2>送料区分一覧</h2>
        {{ $shippings->links() }}
        <div class="main-list list-group">
            @foreach($shippings as $shipping)
                <div href="" class="list-group-item list-group-item-action" data-bs-toggle="modal" data-bs-target="#testModal" data-shipping-id={{ $shipping->id }} data-name="{{ $shipping->name }}" data-sections="{{ $shipping->sections }}" data-display="{{ $shipping->display }}">
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-footer">
					{!! Form::open(['url' => route('admin.shippings.destroy', 'number'), 'method' => 'delete', 'class' => 'd-inline delete-btn']) !!}
	...
	<script>
		$(function(){
			$('.list-group-item').click(function(){
				...
				$edit_url = @json(route('admin.shippings.edit', 'number'));
				$edit_url = $edit_url.replace('number', $shipping_id);
				$('#testModal').find('.edit-btn').attr('href', $edit_url);

				$delete_url = @json(route('admin.shippings.destroy', 'number'));
