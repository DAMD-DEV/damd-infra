...
@section('main')
    <h1>仕様/グレード</h1>
    <div class="clearfix">
        <a href="{{ route('admin.specs.create') }}" class="btn btn-primary float-end">追加</a>
    </div>
    <div>
        <h2>仕様/グレード一覧</h2>
        {{ $specs->links() }}
        <div class="main-list list-group">
            @foreach($specs as $spec)
                <div href="" class="list-group-item list-group-item-action" data-bs-toggle="modal" data-bs-target="#testModal" data-spec-id={{ $spec->id }} data-name="{{ $spec->name }}" data-code="{{ $spec->code }}" data-maker-id={{ $spec->maker_id }} data-car-id={{ $spec->car_id }} data-display="{{ $spec->display }}">
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-footer">
					{!! Form::open(['url' => route('admin.specs.destroy', 'number'), 'method' => 'delete', 'class' => 'd-inline delete-btn']) !!}
	...
	<script>
		$(function(){
			$('.list-group-item').click(function(){
				...
				$edit_url = @json(route('admin.specs.edit', 'number'));
				$edit_url = $edit_url.replace('number', $spec_id);
				$('#testModal').find('.edit-btn').attr('href', $edit_url);
				
				$delete_url = @json(route('admin.specs.destroy', 'number'));
