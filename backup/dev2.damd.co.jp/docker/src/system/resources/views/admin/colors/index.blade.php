...
@section('main')
    <h1>塗装色</h1>
    <div class="clearfix">
        <a href="{{ route('admin.colors.create') }}" class="btn btn-primary float-end">追加</a>
    </div>
    <div>
        <h2>塗装色一覧</h2>
        {{ $colors->links() }}
        <div class="main-list list-group">
            @foreach($colors as $color)
                <div href="" class="list-group-item list-group-item-action" data-bs-toggle="modal" data-bs-target="#testModal" data-color-id={{ $color->id }} data-name="{{ $color->name }}" data-code="{{ $color->code }}" data-display="{{ $color->display }}">
		...
		<div class="modal-dialog modal-lg">
			<div class="modal-content">
				...
				<div class="modal-footer">
					{!! Form::open(['url' => route('admin.colors.destroy', 'number'), 'method' => 'delete', 'class' => 'd-inline delete-btn']) !!}
	...
	<script>
		$(function(){
			$('.list-group-item').click(function(){
				...
				$edit_url = @json(route('admin.colors.edit', 'number'));
				$edit_url = $edit_url.replace('number', $color_id);
				$('#testModal').find('.edit-btn').attr('href', $edit_url);

				$delete_url = @json(route('admin.colors.destroy', 'number'));
