...
@section('main')
    <h1>車種</h1>
    <div class="clearfix">
        <a href="{{ route('cars.create') }}" class="btn btn-primary float-end">追加</a>
    </div>
    <div>
        <h2>車種一覧</h2>
        {{ $cars->links() }}
        <div class="main-list list-group">
            @foreach($cars as $car)
                <div href="" class="list-group-item" data-bs-toggle="modal" data-bs-target="#testModal" data-car-id={{ $car->id }} data-name="{{ $car->name }}" data-code="{{ $car->code }}" data-maker-id={{ $car->maker_id }} data-display="{{ $car->display }}">
