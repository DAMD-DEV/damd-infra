...
@section('main')
    <h1>商品カテゴリ</h1>
    <div class="clearfix">
        <a href="{{ route('categories.create') }}" class="btn btn-primary float-end">追加</a>
    </div>
    <div>
        <h2>商品カテゴリ一覧</h2>
        {{ $categories->links() }}
        <div class="main-list list-group">
            @foreach($categories as $category)
                <div href="" class="list-group-item" data-bs-toggle="modal" data-bs-target="#testModal" data-category-id={{ $category->id }} data-name="{{ $category->name }}" data-code="{{ $category->code }}" data-display="{{ $category->display }}">
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-footer">
					{!! Form::open(['url' => route('categories.destroy', 'number'), 'method' => 'delete', 'class' => 'd-inline delete-btn']) !!}
	...
	<script>
		$(function(){
			$('.list-group-item').click(function(){
				...
				$edit_url = @json(route('categories.edit', 'number'));
				$edit_url = $edit_url.replace('number', $category_id);
				$('#testModal').find('.edit-btn').attr('href', $edit_url);

				$delete_url = @json(route('categories.destroy', 'number'));
